...
// 生成PES头中的PTS和DTS字段
static void make_pes_pts(uint8_t* buff, uint64_t ts)
{
	// PTS: pts_dts_flag = 10,   0010头
	buff[0] = (uint8_t)(((ts >> 30) & 0x07) << 1) | 0x20 | 0x01;
	buff[1] = (uint8_t)((ts >> 22) & 0xff);
	buff[2] = (uint8_t)(((ts >> 15) & 0xff) << 1) | 0x01;
	buff[3] = (uint8_t)((ts >> 7) & 0xff);
	buff[4] = (uint8_t)((ts & 0xff) << 1) | 0x01;
...
int lts_pes_make_header(uint8_t stream_id, uint64_t pts, int es_len, uint8_t *dest, int maxlen)
{
	...
	// prefix,前三字节是包头起始标识字段
	*buf++ = 0;
	*buf++ = 0;
	*buf++ = 1;

	// stream_id,第四个字节是流ID字段
	*buf++ = stream_id;	

	if (stream_id == 0xE0)
	{
		// pes length,第五六个字节是PES包长度
	...
	{
		pes_packet_length = pes_packet_length - 5;
		// pes length,第五六个字节是PES包长度
		*buf++ = (uint8_t)(pes_packet_length >> 8);
		*buf++ = (uint8_t)pes_packet_length;
